...
class IslandPearlSell(Island):
    ...
    def rank_price_button_from_visit(self, visit_button, index):
        ...
        return self._area_button(area, f'OCR_ISLAND_PEARL_RANK_PRICE_FROM_VISIT_{index}')
    ...
    def ocr_current_pearl_count(self):
        """OCR 当前持有珍珠数量。"""
        self.device.screenshot()
        count = self._ocr_digit(OCR_ISLAND_PEARL_CURRENT_COUNT, name='pearl_current_count')
    ...
    def _ocr_digit(self, button, name):
        ocr = Digit(button, lang='cnocr', letter=(255, 255, 255), threshold=128, name=name)
        try:
    ...
    def _ocr_text(self, button, name):
        ocr = Ocr(button, lang='cnocr', letter=(255, 255, 255), threshold=128, name=name)
        try:
